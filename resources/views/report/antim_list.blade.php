...
@section('styles')
    ...
    <style>
        ...
        #antimListTable thead th {
            vertical-align: middle;
            font-weight: 600;
            color: #fff;
        }

        #antimListTable tbody tr:hover {
...
@section('body-page')
    <main id="main" class="main">
        <div class="pagetitle">
            <h5 class="fw-bold">अंतिम सूची </h5>
            <nav>
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">अंतिम सूची </li>
                </ol>
            </nav>
        </div>
                        ...
                        <ul class="nav nav-tabs card-header-tabs" id="reportTabs" role="tablist">
                            <li class="nav-item" role="presentation">
                                <button class="nav-link active" id="download-tab" data-bs-toggle="tab"
                                    ...
                                    <i class="bi bi-download"></i> अंतिम सूची डाउनलोड करें
                                </button>
                            </li>
                            <li class="nav-item" role="presentation">
                                <button class="nav-link" id="upload-tab" data-bs-toggle="tab" data-bs-target="#upload"
                                    type="button" role="tab">
                                    <i class="bi bi-upload"></i> अंतिम सूची अपलोड करें
                                </button>
                            </li>
                        </ul>
                    </div>
                    ...
                    <div class="card-body tab-content">
                        ...
                        <div class="tab-pane fade show active" id="download" role="tabpanel">
                            <form id="downloadForm">
                                 <div class="mb-3 mt-2">
                                    <label class="form-label">रिपोर्ट डाउनलोड विकल्प <span class="text-danger">*</span></label>
                                    <select name="reportdownloadOption" id="reportdownloadOption" class="form-control" required>
                                ...
                                {{-- <div class="row mb-3">
                                    ...
                                    <div class="col-md-6">
                                        <label class="form-label fw-bold">दावा आपत्ति की वैधता तिथि <span
                                                class="text-danger">*</span></label>
                                        <input type="date" class="form-control" name="claim_end_date" id="ClaimEndDate"
                                            required>
                                    </div>
                                </div> --}}
                                <div class="mb-3">
                                    <label class="form-label fw-bold">फ़ाइल अपलोड करें <span
                                            class="text-danger">*</span></label>
                                    <input type="file" class="form-control" accept=".pdf" name="antim_file"
                                        id="upload_antim_file" required>
                                </div>

                                <div class="form-check mb-3">
                                    <input class="form-check-input border border-primary" type="checkbox" id="confirm_upload" name="confirm_upload" required>
                                    <label class="form-check-label" for="confirm_upload">क्या आप अंतिम सूची अपलोड करना चाहते हैं ?</label>
                                </div>
        ...
        <div class="card mt-3" id="antimTableCard" style="display: none;">
            <div class="card-header">
                <h5 class="card-title mb-0">अंतिम सूची विवरण</h5>
            </div>
            <div class="card-body table-responsive">
                <table class="table table-hover table-bordered w-100" id="antimListTable">
                                ...
                                {{-- <td>
                                    <div class="btn-group btn-group-sm" role="group">
                                        <button type="button" class="btn btn-outline-primary view-pdf-btn"
                                            data-pdf="{{ asset('uploads/' . $item->antim_list_file) }}"
                                            data-title="{{ $item->postname }} - {{ $item->Advertisement_Title }}"
                                            title="देखें"><i class="bi bi-eye"></i> देखें
                                        </button>
                                        <a href="{{ asset('uploads/' . $item->antim_list_file) }}"
                                            class="btn btn-outline-success" download title="डाउनलोड करें">
                                            <i class="bi bi-download"></i> डाउनलोड
                                        </a>
                                    </div>
                                </td> --}}
                                @php
                                    $fileExists = !empty($item->antim_list_file);
                                    $file_path = '';
                                    if ($fileExists) {
                                        $file_path = asset('uploads/' . $item->antim_list_file);
                                        if (config('app.env') === 'production') {
                                            $file_path = config('custom.file_point') . $item->antim_list_file;
                                        }
                                    }
                                @endphp
                                <td>
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="text-center py-4">
                                    ...
                                    <p class="text-muted mt-2">कोई अंतिम सूची उपलब्ध नहीं है।</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
...
@section('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            // Initialize DataTable only once when needed
            let antimTable = null;
            let tableInitialized = false;

            function initializeDataTable() {
                if (!tableInitialized && $.fn.DataTable.isDataTable('#antimListTable') === false) {
                    antimTable = $('#antimListTable').DataTable({
                        responsive: true,
                        // language: {
                        //     "search": "खोजें:",
            ...
            // Enable download button when post is selected
            // Manage upload button enabling: require both file and confirmation checkbox
            function updateUploadButtonState() {
                const fileSelected = $('#upload_antim_file').get(0).files.length > 0;
                const confirmed = $('#confirm_upload').is(':checked');
                const UploadButton = $('#UploadButton');
                UploadButton.prop('disabled', !(fileSelected && confirmed));
            }

            $('#upload_antim_file, #confirm_upload').on('change', updateUploadButtonState);
            ...
            $('#btnDownloadReport').on('click', function() {
                ...
                window.location.href =
                    `{{ route('report.downloadPostWiseReport') }}?advertisement_id=${advertisementId}&post_id=${postId}&is_antim_list=true&reportdownloadOption=${reportdownloadOption}`;
            ...
            $('#uploadForm').on('submit', function(e) {
                ...
                const fileInput = $('#upload_antim_file')[0];

                // ================= VALIDATIONS =================

                if (!advertisementId || !postId || !fileInput.files.length) {
                    Swal.fire({
                        icon: 'warning',
                        title: 'अधूरी जानकारी',
                ...
                if (!$('#confirm_upload').is(':checked')) {
                    Swal.fire({
                        icon: 'warning',
                        title: 'कन्फर्मेशन आवश्यक है',
                        text: 'कृपया पुष्टि करें कि आप अंतिम सूची अपलोड करना चाहते हैं।',
                        confirmButtonText: 'ठीक है'
                    });
                    return;
                }
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const tableCard = document.getElementById('antimTableCard');
        ...
        function setupPdfViewer() {
            ...
            // Use event delegation in case buttons are rendered dynamically
