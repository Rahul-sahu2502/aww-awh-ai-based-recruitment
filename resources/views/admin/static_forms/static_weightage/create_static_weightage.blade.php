            ...
            <div class="col-lg-12 col-md-12 col-xs-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form action="{{ route('admin.static-weightage.store') }}" method="post" id="weightageForm">
                            @csrf

                            <div class="row mb-4">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label for="post_config_id" class="form-label">पद चुनें <span
                                                class="text-danger">*</span></label>
                                        <select class="form-select @error('post_config_id') is-invalid @enderror"
                                            name="post_config_id" id="post_config_id" required>
                                            <option value="">-- पद चुनें --</option>
                                            @foreach ($posts as $post)
                                                @if (is_object($post) && isset($post->id) && isset($post->title))
                                                    <option value="{{ $post->id }}"
                                                        {{ request()->get('post_config_id') == $post->id || old('post_config_id') == $post->id || $post_config_id == $post->id ? 'selected' : '' }}>
                                                        {{ $post->title }}
                                                    </option>
                                                @endif
                                            @endforeach
                                        </select>
                                        @error('post_config_id')
                            ...
                            <div id="qualificationsContainer" style="display:none;">
                                <div class="card mt-4">
                                    ...
                                    <div class="card-body">
                                        <p class="text-muted">इस पद के लिए निर्धारित योग्यताओं के अनुसार अंक सेट करें:</p>
                                        <div class="row" id="qualificationsInputs">
                                            <!-- Will be populated dynamically based on selected post -->
                                            <div class="alert alert-info">कृपया पहले पद चुनें। पद-विशिष्ट योग्यताएं यहां
                                                दिखाई जाएंगी।</div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div id="casteMarksContainer" style="display:none;">
                                <div class="card mt-4">
                                    ...
                                    <div class="card-body">
                                        <p class="text-muted">इस पद के लिए निर्धारित जातियों के अनुसार अंक सेट करें:</p>
                                        <div class="row" id="casteMarksInputs">
                                            <!-- Will be populated dynamically based on selected post -->
                                            <div class="alert alert-info">कृपया पहले पद चुनें। पद-विशिष्ट जातियां यहां
                                                दिखाई जाएंगी।</div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="form-group">
                                ...
                                <a href="{{ route('admin.static-weightage.index') }}" class="btn btn-secondary">
...
@section('scripts')
    <script>
        $(document).ready(function() {
            ...
            $('#post_config_id').change(function() {
            ...
            // If post_config_id is already selected on page load (from query param)
            if ($('#post_config_id').val()) {
                fetchQuestions($('#post_config_id').val());
            }
            ...
            function fetchQuestions(postId) {
                ...
                $.ajax({
                    url: "{{ route('admin.static-weightage.questions', '') }}/" + postId,
                    type: 'GET',
                    success: function(response) {
                        ...
                        // Caste data is now handled in displayQuestions function
                    },
                    error: function(xhr) {
                        $('#questionsContainer').html(
            ...
            function displayQuestions(data) {
                ...
                // Update the castes container with post-specific castes
                let casteHtml = '';
                const castes = data.castes || [];
                const existingCasteMarks = data.existingCasteMarks || {};

                if (castes && castes.length > 0) {
                    castes.forEach(function(caste) {
                        const existing = existingCasteMarks[caste.caste_id] ?
                            parseInt(existingCasteMarks[caste.caste_id].marks) : '';
                        casteHtml += `
                            <div class="col-md-3 col-sm-6">
                                <div class="mb-2">
                                    ...
                                    <input type="number" class="form-control" 
                                        ...
                                        name="caste_marks[${caste.caste_id}]" 
                                        step="1" min="0" 
                    ...
                    casteHtml =
                        '<div class="col-12"><div class="alert alert-info">इस पद के लिए कोई विशिष्ट जाति निर्धारित नहीं है।</div></div>';
                }

                // Update the castes container
